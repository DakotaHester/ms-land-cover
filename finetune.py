    ...
    if args.model == 'unet':
        ...
        if args.encoder_weights is not None:
            if args.encoder_weights == 'imagenet':
                ...
                weights = load_pth(args.encoder_weights)
                weights = {k: v for k, v in weights.items() if 'segformer.encoder' in k}
                weights = {k.replace('segformer.encoder.', ''): v for k, v in weights.items()}
                model.segformer.encoder.load_state_dict(weights)
        ...
        if args.freeze_decoder:
            ...
            # do not freeze the classifier
            for param in model.decode_head.classifer.parameters():
