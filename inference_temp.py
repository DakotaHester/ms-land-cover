...
def main():
    ...
    kernel_size = 7
    logger.log('Filtering land cover classes...')
    lc_classes_filtered = fast_mode_filter(lc_classes, size=kernel_size)
    
    logger.log('Saving filtered land cover classes...')
    with rio.open(os.path.join(out_path, 'lc_classes_filtered.tif'), 'w', **lc_classes_profile) as dst:
        dst.write(lc_classes_filtered, 1)
        dst.write_colormap(1, cmap)
    logger.log(f'Saved filtered land cover classes to {os.path.join(out_path, "lc_classes_filtered.tif")}')
    
    # clip filtered land cover classes
    logger.log('Clipping filtered land cover classes...')
    with rio.open(os.path.join(out_path, 'lc_classes_filtered.tif')) as src:
        clipped_data, clipped_transform = mask(
            src,
            bounding_polygons, 
            crop=True,
        ...
        with rio.open(os.path.join(out_path, 'lc_classes_filtered_clipped.tif'), 'w', **updated_profile) as dst:
            dst.write(clipped_data)
            dst.write_colormap(1, cmap)
