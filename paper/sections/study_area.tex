...
The total size of the NAIP imagery used in this work was \SI[number-unit-product={\text{ }}]{599.4}{\giga \byte}, comprising 82 county-level GeoTiff files, an average of \SI[number-unit-product={\text{ }}]{7.31}{\giga \byte} per county, with individual county rasters ranging in size from \SI[number-unit-product={\text{ }}]{3.861}{\giga \byte} to \SI[number-unit-product={\text{ }}]{13.433}{\giga \byte}.
A visualization of the NAIP imagery used over the study area is provided in Figure~\ref{fig:study_area}.
We selected three-band images for compatibility with BYOL, which is designed for three-channel inputs, and for easy comparison with models using ImageNet pre-trained weights, which also expect three-channel inputs.
...
The stratification approach proposed in this work (Section~\ref{sec:methodology:dataset_curation}) relies on an existing land cover product to select strata from which training samples are drawn.
