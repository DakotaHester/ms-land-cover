...
Similarly, the intensity of the Poisson noise is controlled by the rate parameter \(\lambda\), which is also randomly sampled from a uniform distribution \(U(0, 2)\). % NOTE: THIS HAS BEEN CHANGED IN NEWER EXPERIMENTS TO 0,1 

% Since random variables sampled from a Poisson distribution are strictly non-negative, Poisson noise is both added and subtracted from the input image (using the same \(\lambda\) value) to increase the effect of the noise. % NOTE: THIS IS ONLY TRUE IN NEWER EXPERIMENTS
