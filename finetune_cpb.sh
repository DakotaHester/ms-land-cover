...
BASE_LOG_DIR="./logs/finetune_cpb_20250816"
BASE_WEIGHTS_DIR="./weights/finetune_cpb_20250816"
SLURM_SCRIPT_DIR="./slurm_scripts/finetune_cpb_20250816"
    ...
    for strategy in "${STRATIFICATION_SCHEMES[@]}"; do
        for n_train in "${N_TRAIN_SAMPLES[@]}"; do
            for fold in $(seq 1 $N_FOLDS); do
                ...
                ENCODER_WEIGHTS="$PRETRAIN_SCHEME"
                
                SPLIT_DIR="./data/cpb_tests/splits/rep_${rep}/${strategy}"

                # Unique log/output directories for this job
                JOB_NAME="${MODEL}_${PRETRAIN_SCHEME}_${strategy}_n${n_train}_rep${rep}_fold${fold}"
                BASE_DIR="${MODEL}/${PRETRAIN_SCHEME}/${strategy}/n_${n_train}/rep_${rep}/fold_${fold}"
...
echo "Model: $MODEL"
echo "Pretrain scheme: $PRETRAIN_SCHEME"
...
EOL
                    ...
                    run_python_job "$MODEL" "$PRETRAIN_SCHEME" "$n_train" "$fold" "$freeze_encoder" "$ENCODER_WEIGHTS" "$JOB_LOG_DIR" "$JOB_WEIGHTS_DIR" "$SPLIT_DIR"
                    COUNT=$((COUNT+1))
...
```// filepath: /Users/dak/projects/ms-land-cover/finetune_cpb.sh
#!/bin/bash
